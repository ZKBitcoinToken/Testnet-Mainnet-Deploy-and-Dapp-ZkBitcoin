...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const contractAddressLP = "0x80e8Ae8e73f60130EA05e579A23fD507F40ad713";
  ...
  // Create deployer object and load the artifact of the contract you want to deploy.
  const artifactSTAKING = await deployer.loadArtifact("zkBitcoinStakingETH");
...
const stringz = "yarn hardhat verify --network zkSyncTestnet "+contractAddressSTAKING+` "`+contractAddressZKBTC+`" "`+contractAddressLP+`" "`+contractAddress0xBTC+`" "`+contractAddressAuctions+`" --contract "contracts/zkBitcoinStakingETH.sol:zkBitcoinStakingETH"`;
console.log("FINISHED");
console.log("zkBTC Contract: ", contractAddressZKBTC);
